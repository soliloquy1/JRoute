 ...
 * Files that do not exist in this tree are skipped (not failed),
 * since deleted OmniRoute modules are not regressions.
 */
const budget = [
...
for (const item of budget) {
  ...
  if (!fs.existsSync(absolutePath)) {
    // File was intentionally removed from this tree — not a regression.
    skipped += 1;
    continue;
  }
...
if (skipped > 0) {
  console.log(`[t11:any-budget] ${skipped} file(s) not present in this tree — skipped.`);
}

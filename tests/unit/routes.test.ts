...
// Important 2: without the .filter((p) => p.enabled) the route advertises every disabled
// backend to any anonymous caller, and the test above stays green because it only seeds
// an enabled provider and only asserts presence.
test("GET /v1/models OMITS disabled providers", async () => {
  seedOpenAi();
  upsertProvider({
    id: "disabled-backend",
  ...
  assert.ok(
    !body.data.some((m) => m.id === "disabled-backend"),
    "a disabled provider must never be advertised"
  );
  assert.ok(
    body.data.some((m) => m.id === "openai"),
    "enabled providers must still list"
  );
});
...
test("GET /v1/models emits the exact OpenAI model row shape", async () => {
  ...
  const row = body.data.find((m) => m.id === "openai");
  assert.deepEqual(row, { id: "openai", object: "model", owned_by: "jroute" });
});
